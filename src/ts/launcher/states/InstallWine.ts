...
export default (launcher: Launcher): Promise<void> => {
    return new Promise(async (resolve) => {
        Runners.download('lutris-GE-Proton7-10-x86_64').then((stream) => {
            launcher.progressBar?.init({
                label: 'Downloading Wine-GE-Proton 7-10...',
                showSpeed: true,
                showEta: true,
                showPercents: true,
                showTotals: true
            ...
            stream?.unpackStart(() => {
                launcher.progressBar?.init({
                    label: 'Unpacking Wine-GE-Proton 7-10...',
                    showSpeed: true,
                    showEta: true,
                    showPercents: true,
                    showTotals: true
            ...
            stream?.unpackFinish(async () => {
                // Select this runner
                await Runners.current('lutris-GE-Proton7-10-x86_64');
